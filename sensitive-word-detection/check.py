...
num_of_output_file = 3
max_num_of_check_len = 4
...
f = None
fname = "./output.txt"
for i in range(num_of_output_file):
    fname = f"output{i+1}.txt"
    with open(fname, "r", encoding="utf-8") as f:
        list_temp = f.read().split("\n")
    lis += list(set(list_temp))
...
with open("whitelist.txt", "r", encoding="utf-8") as f:
    list_temp = f.read().split("\n")
    lis = list(set(lis) - set(list_temp))
